...
const HomeScreen = ({ route, navigation }) => {
  ...
  const userTypeNames = {
    admin: 'Admin',
    garbageCollector: 'Coletor',
    publicUser: 'Usuário Público',
  };

  const handleSignOut = () => {
    // Executei a função de logout aqui
    navigation.navigate('SignIn'); // Navegue até a tela de login
  };

  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>RotaLimpa</Text>
      </View>
      <View style={styles.header}>
      <Text style={styles.userType}>Bem vindo {userTypeNames[userType]}</Text>
      </View>
      ...
      <View style={styles.buttonContainer}>
        ...
        {(userType === 'admin') && (
          ...
          >
            ...
            <Text style={styles.buttonText}>Ver Report</Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity
